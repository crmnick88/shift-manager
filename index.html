...
.approval-buttons{ display:inline-flex; gap:10px; margin-right:10px; flex-wrap:wrap; }
    ...
    /* =======================
       📦 Manager constraints - compact grid layout
       ======================= */
    .dept-grid{
      column-count:2;
      column-gap:14px;
    ...
    @media (max-width: 900px){
      .dept-grid{ column-count:1; }
    }
    @media (min-width: 1400px){
      .dept-grid{ column-count:3; }
    }
    .dept-card h3{
      margin:0 0 10px 0 !important;
    }

    /* Make employee blocks more compact */
    .employee-constraints{
    ...
    /* constraint rows inside employee card */
    .employee-constraints > div[style*="margin:10px 0"]{
...
/* =======================
   🔵 BIGGER LOGIN SCREEN (mobile friendly)
   ======================= */
#login-screen .mode-btn{
  font-size:22px;
  padding:22px 30px;
  width:90%;
  max-width:360px;
...
/* =======================
   🟦 LOGIN BIG (strong override)
   ======================= */
#employee-login, #manager-login{
  padding: 16px !important;
}

#employee-login > div, #manager-login > div{
  width: min(520px, 96vw) !important;
  max-width: 96vw !important;   /* overrides inline max-width:400px */
  margin: 18px auto !important;
}

#employee-login label, #manager-login label{
  font-size: 18px !important;
}

#emp-username, #emp-password, #mgr-username, #mgr-password{
  width: 100% !important;
  font-size: 20px !important;   /* 16+ prevents mobile zoom, 20 is comfy */
  padding: 18px 16px !important;
  border-radius: 14px !important;
}

#employee-login .btn, #manager-login .btn{
  width: 100% !important;
...
/* bigger headings on phone */
@media (max-width: 600px){
  #employee-login h1, #manager-login h1{
    ...
    /* =======================
       📱 Small screens - make container fill more space
       ======================= */
    @media (max-width: 600px){
...
<div class="container">
  ...
  <div id="login-screen" class="section active">
    ...
    <div style="text-align:center; margin-top:28px;">
      <button class="mode-btn" onclick="showLoginForm('employee')">👤 כניסת עובד</button>
      <button class="mode-btn" onclick="showLoginForm('manager')">👔 כניסת מנהל</button>
    </div>
  </div>
  ...
  <div id="employee-login" class="section">
    ...
    <div style="width:95%; margin:24px auto;">
      ...
      <button class="btn" onclick="loginEmployee()">התחבר</button>
      <button class="btn" onclick="backToLogin()" style="background:#6c757d;">חזור</button>
    </div>
  </div>
  ...
  <div id="manager-login" class="section">
    ...
    <div style="width:95%; margin:24px auto;">
      ...
      <button class="btn" onclick="loginManager()">התחבר</button>
      <button class="btn" onclick="backToLogin()" style="background:#6c757d;">חזור</button>
    </div>
  </div>
  ...
  <div id="employee-section" class="section">
    ...
    <div style="background:#f8f9fa; padding:20px; border-radius:12px;">
      <h3>הגדרת אילוצים (עד 2)</h3>
      <p style="color:#e74c3c; margin:10px 0;">⚠️ לא ניתן לבחור שישי או מוצ"ש</p>
      ...
      <div class="constraint-item">
        ...
        <div class="form-group">
          ...
          <select id="c1-type">
            ...
            <option value="no-morning">לא בוקר</option>
            <option value="no-evening">לא ערב</option>
      ...
      <button class="btn" onclick="saveConstraints()">שמור אילוצים</button>
    </div>
  ...
  async function initPushNotifications() {
    try {
      ...
      messaging.onMessage((payload) => {
        try {
          const n = payload && payload.notification ? payload.notification : null;
          if (n && Notification.permission === 'granted') {
            new Notification(n.title || 'התראה', {
              body: n.body || '',
              icon: './icon-192.png'
            });
          }
...
async function approveCurrentSchedule(){
 ...
 function logout() {
  ...
  async function loadAllConstraints() {
    ...
    let html = '<h2>אילוצי העובדים</h2><div class="dept-grid">';
      ...
      employees.forEach(emp => {
        ...
        html += `
          <div class="emp-header-row">
            ...
            <button class="reset-btn" onclick="resetEmployeeConstraints('${DISPLAY_NAMES[emp] || emp}')">🧹 איפוס אילוצים לעובד</button>
          </div>
        `;
  ...
  function formatConstraintForManager(emp, key, c) {
    ...
    if (c.type === 'day-off') {
      ...
      if (status === 'pending') {
        html += `<div class="approval-buttons">
          <button class="approve-btn" onclick="updateApproval('${DISPLAY_NAMES[emp] || emp}','${key}','approved')">✅ אשר</button>
          <button class="reject-btn" onclick="updateApproval('${DISPLAY_NAMES[emp] || emp}','${key}','rejected')">❌ דחה</button>
        </div>
    ...
    html += `
      <div class="approval-buttons" style="margin-top:8px;">
        <button class="reset-btn" onclick="deleteSingleConstraint('${DISPLAY_NAMES[emp] || emp}','${key}')">🗑️ מחק אילוץ זה</button>
      </div>
    `;
    ...
    deptOrder.forEach(dept => {
      ...
      employees.forEach(emp => {
        ...
        dayCols.forEach(c => {
          ...
          const shifts = [];
          if (day.morning === emp) shifts.push(isKavLavan ? '<span class="shift-morning">בוקר</span>' : '<span class="shift-morning">בוקר<br>9:00-16:00</span>');
          if (day.middle === emp) shifts.push(isKavLavan ? '<span class="shift-middle">אמצע</span>' : '<span class="shift-middle">אמצע<br>11:00-19:00</span>');
          if (day.evening === emp) shifts.push(isKavLavan ? '<span class="shift-evening">ערב</span>' : '<span class="shift-evening">ערב<br>15:00-21:30</span>');
...
<head>
  ...
  border-collapse: collapse;
  width: 100%;
  table-layout: fixed;   /* ⭐ הכי חשוב – רוחב קבוע כמו אקסל */
}

th, td{
  border:1px solid #ccc;
  padding:2px;           /* פחות רווח פנימי */
  text-align:center;
  width:70px;            /* רוחב תא קטן */
  height:32px;           /* גובה כמו באקסל */
  font-size:11px;        /* טקסט קומפקטי */
...
<body>
  ...
  <table>
    ...
    deptOrder.forEach((dept, deptIndex) => {
      ...
      if (deptIndex > 0) {
        html += `<tr class="dept-row"><td colspan="${sampleSchedule.length + 1}">🏢 ${dept}</td></tr>`;
      }
            ...
            if (day.friday) {
              if (isThreeEmp) {
                if (Array.isArray(day.friday) && day.friday.includes(emp)) {
                  cellText = isKavLavan ? 'עובד בשישי' : 'שישי<br>8:30-14:30';
                  cellClass = 'friday';
                } else {
                  cellText = '-';
                  cellClass = 'empty';
              ...
              if (isThreeEmp) {
                if (day.saturday === emp) {
                  cellText = isKavLavan ? 'עובד במוצ"ש' : 'מוצ"ש<br>מוצ"ש-22:30';
                  cellClass = 'saturday';
                } else {
                  cellText = '-';
                  cellClass = 'empty';
              ...
              const shifts = [];
              if (day.morning === emp) shifts.push({ text: isKavLavan ? 'בוקר' : 'בוקר<br>9:00-16:00', class: 'morning' });
              if (day.middle === emp) shifts.push({ text: isKavLavan ? 'אמצע' : 'אמצע<br>11:00-19:00', class: 'middle' });
              if (day.evening === emp) shifts.push({ text: isKavLavan ? 'ערב' : 'ערב<br>15:00-21:30', class: 'evening' });
        ...
        sampleSchedule.forEach(dayCol => {
          ...
          if (day) {
            if (day.friday) {
              if (isThreeEmp) {
                if (Array.isArray(day.friday) && day.friday.includes(emp)) {
                  cellText = isKavLavan ? 'עובד בשישי' : 'שישי 8:30-14:30';
                } else {
                  cellText = '-';
                }
              } else {
                cellText = day.friday === emp ? 'בוקר 8:30-14:30' : '-';
              }
            } else if (day.saturday) {
              if (isThreeEmp) {
                cellText = day.saturday === emp 
                  ? (isKavLavan ? 'עובד במוצ"ש' : 'מוצ"ש מוצ"ש-22:30')
                  : '-';
              } else {
                cellText = day.saturday === emp ? 'ערב מוצ"ש-22:30' : '-';
              }
            } else {
              const shifts = [];
              if (day.morning === emp) shifts.push(isKavLavan ? 'בוקר' : 'בוקר 9:00-16:00');
              if (day.middle === emp) shifts.push(isKavLavan ? 'אמצע' : 'אמצע 11:00-19:00');
              if (day.evening === emp) shifts.push(isKavLavan ? 'ערב' : 'ערב 15:00-21:30');
              if (shifts.length >= 2) cellText = 'כפולה';
...
// ✅ Merge department rows once (outside styling loops)
ws['!merges'] = [];
for (let r = 0; r < data.length; r++) {
  if (data[r][0] && typeof data[r][0] === 'string' && data[r][0].startsWith('🏢')) {
    ws['!merges'].push({
      s: { r: r, c: 0 },
      e: { r: r, c: headerRow.length - 1 }
    });
  }
}
...
// Department divider rows
        else if (data[R][0] && data[R][0].startsWith('🏢')) {
          ws[cellAddress].s.fill = {
            fgColor: { rgb: '667EEA' }
          };
          ws[cellAddress].s.font = {
...
// Employee name column
        ...
        // Shift cells - color based on content
          ...
          } else if (cellValue.includes('בוקר')) {

            ws[cellAddress].s.fill = { fgColor: { rgb: 'FFF3CD' } };
          } else if (cellValue.includes('אמצע')) {
            ws[cellAddress].s.fill = { fgColor: { rgb: 'FFD4A3' } };
          } else if (cellValue.includes('ערב') && !cellValue.includes('שישי') && !cellValue.includes('מוצ')) {
            ws[cellAddress].s.fill = { fgColor: { rgb: 'D1ECF1' } };
          } else if (cellValue.includes('שישי')) {
            ws[cellAddress].s.fill = { fgColor: { rgb: 'D4EDDA' } };
          } else if (cellValue.includes('מוצ')) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saved === 'MANAGER') {
    ...
    hideAll();
    document.getElementById('employee-section').classList.add('active');
    document.getElementById('employee-welcome').textContent = `שלום ${saved}! 👋`;
    applyConstraintOptions(currentEmployee);
    loadEmployeeConstraints();
  }
